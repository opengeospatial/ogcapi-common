...
include::./recommendations/filterable-collections/REC_filtering-queryables.adoc[]
...
[[filtering-filter-crs-parameter]]
=== Query Parameter `filter-crs`

The `filtering-crs` parameter specifies the Coordinate Reference System (CRS) in which the `filter` query parameter is provided, in the form of a URI or safe CURIE.

include::./requirements/filterable-collections/REQ_filtering-filter-crs-parameter.adoc[]

include::./recommendations/filterable-collections/REC_filtering-supported-crs.adoc[]

NOTE: For the use case of filtering collections by a specific CRS rather than only including collections within a particular extent, the server
could provide a `crs` queryable that could be the list of supported output CRSs, for example, `/collections?filter=A_CONTAINS(crs,'[EPSG:3035]')`
(assuming that queryable is documented as being an array and using safe CURIEs).
