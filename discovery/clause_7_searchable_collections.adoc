...
This "Searchable Collections" defines four query parameters which can be used with the `/collections` endpoint (described in the https://docs.ogc.org/DRAFTS/20-024.html#rc-collections-section[Collections Requirements Class of OGC API - Common - Part 2: Geospatial Data])
...
* <<searchable-query-parameter,query>>: Like `q`, but with the possibility to search for occurences of multiple terms being present together (`+`) or excluding one or more terms (`-`)
...
This "Searchable Collections" requirement class is consistent with the requirements of https://docs.ogc.org/is/20-004r1/20-004r1.html#clause-local-resources-catalog_query-parameters[OGC API - Records - Part 1: Core "Local Resources Catalog - Query Parameters" (Deployment)].
...
| https://docs.ogc.org/is/20-004r1/20-004r1.html#clause-record-core-query-parameters[Query Parameters]                                       | https://www.opengis.net/spec/ogcapi-records-1/1.0/conf/record-query-parameters
| https://docs.ogc.org/is/20-004r1/20-004r1.html#clause-local-resources-catalog_query-parameters[Local Resources Catalog - Query Parameters] | https://www.opengis.net/spec/ogcapi-records-1/1.0/conf/local-resources-catalog-query-parameters
|===
...
[[searchable-bbox-crs-parameter]]
=== Query Parameter `bbox-crs`

The `bbox-crs` parameter specifies the Coordinate Reference System (CRS) in which the `bbox` query parameter is provided, in the form of a URI or safe CURIE.

include::requirements/searchable-collections/REQ_searchable-bbox-crs.adoc[]

include::./recommendations/searchable-collections/REC_searchable-supported-crs.adoc[]

[[searchable-datetime-parameter]]
=== Query Parameter `datetime`

The query `datetime` parameter selects collections based on their temporal extent.
...
The `query` parameter is similar to the `q` parameter, but additionally specifies the use of the `+++` character following a space (or a comma, or as the first character) and appearing before a term to indicate that this additional term must also be present,
...
As another example, `query=canada +weather -extreme,united states +weather -extreme` would return similar results, but also for the United States.

As a counterexample, `query=united +states` would match any collection where both the terms `united` and `states` appear (even if not consecutively).

As another counterexample, `query=united-states` would match a single term `united-states`, rather than being an exclusion.

With `query=canada +extreme weather`, the terms `canada` and `extreme weather` both need to appear, but they do not need to appear consecutively to be a match.

With `query=united,states`, at least one of the following terms need to appear: `united`, `states`.

For more complex queries, the <<rc-filterable-collections-section, Filtering Collections with CQL2 requirements class>> can be used, with more powerful support for logical expressions,
...
[[searchable-sd-resolution-parameters]]
=== Query Parameters `sd` and `resolution`

The `sd` and `resolution` query parameters allow searching for collections usable anywhere within the scale denominator / resolution range specified,
allowing, for example, to filter out low-resolution collections (large minimum cell size) or overly detailed collections not suitable for the intended purpose.
The related properties indicating the minimum and maximum suitable scale denominator in the collection description are `minScaleDenominator` and `maxScaleDenominator`,
as well as the `minCellSize` and `maxCellSize` properties for gridded coverages related via a 0.28 mm/cell assumption.

An empty string or two dots (`..`) can be used on either side of the slash to indicate a half-bounded interval.
A single value without a slash (e.g., `sd=200000` or `resolution=0.000009`) is interpreted as a single target value and evaluates against both minimum and maximum boundaries.

For example, `sd=../200000` (or `sd=/200000`) would only match collections with data intended for a scale of 1:200,000 or larger (e.g., collections intended for 1:2,000,000 or larger would be excluded).

include::requirements/searchable-collections/REQ_searchable-sd.adoc[]

include::requirements/searchable-collections/REQ_searchable-resolution.adoc[]

include::recommendations/searchable-collections/PER_searchable-multiple-scales.adoc[]

NOTE: These requirements imply that collections with unspecified `maxScaleDenominator` and `maxCellSize` are always included when a low query boundary is specified
(e.g., `200000/..`), and collections with unspecified `minScaleDenominator` and `minCellSize` are always included when a high query boundary is specified (e.g., `../5000`).

NOTE: For collections available using data access mechanisms fully supporting generalization / downsampling / overviews, such as OGC API - Tiles or OGC API - DGGS
with support for the coarsest tile matrix / refinement level, the `maxScaleDenominator` and `maxCellSize` would either be omitted from the collection description or correspond
to a very large scale denominator / cell size.
