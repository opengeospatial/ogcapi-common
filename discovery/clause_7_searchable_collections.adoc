...
* <<searchable-query-parameter,query>>: Like `q`, but with the possibility to search for occurences of multiple terms being present together (`+`) or excluding on or more terms (`-`)
...
With `query=canada +extreme weather`, `canada` and `extreme weather` both need to appear, but they do not need to appear consecutively to be a match.
