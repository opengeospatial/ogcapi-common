...
allowing, for example, to filter out low-resolution collections (large minimum cell size) not suitable for the intended purpose.
...
For example, `sd=200000/..` (or simply `sd=200000`) would only match collections with data intended for a scale of 1:200,000 or larger (e.g., collections intended for 1:2,000,000 would be excluded).
...
NOTE: These requirements imply that collections with unspecified `minScaleDenominator` and `minCellSize` are always included in a response where the minimum scale denominator is
specified as `..`, and that collections with unspecified `maxScaleDenominator` and `maxCellSize` are always included in a response where the maximum scale denominator is
specified as `..`.
