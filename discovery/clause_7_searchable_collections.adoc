...
The `sd` and `resolution` query parameters allow searching for collections usable at the scale denominator / resolution range specified,
...
An empty string or two dots (`..`) can be used on either side of the slash to indicate a half-bounded interval. A single value without a slash (e.g., `sd=200000` or `resolution=0.000009`) is interpreted as a single target value and evaluates against both minimum and maximum boundaries.
...
NOTE: These requirements imply that collections with unspecified `minScaleDenominator` and `minCellSize` are always included in a response where the low boundary is left
unbounded (using `..` or an empty string), and that collections with unspecified `maxScaleDenominator` and `maxCellSize` are always included in a response where the high
boundary is left unbounded.
