...
====
[%metadata]
identifier:: /req/searchable-collections/bbox-response
part:: If the `bbox` parameter is provided by the client and supported by the server, then only collections that have a spatial extent that intersects the bounding box and collections that
