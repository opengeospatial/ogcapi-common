...
====
[%metadata]
identifier:: /req/searchable-collections/bbox-definition
part:: The Implementation SHALL support a `bbox` query parameter at the `/collections` endpoint described by the following OpenAPI Specification 3.0 fragment:
+
[source,YAML]
----
name: bbox
...
part:: The Implementation SHALL understand a bounding box provided as four or six numbers, depending on whether the coordinate reference system includes a vertical axis (height or depth):
