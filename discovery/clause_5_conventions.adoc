...
The normative provisions in this Standard are denoted by the URI `https://www.opengis.net/spec/ogcapi-common-4/1.0`.

All <<requirement-definition,Requirements>>, <<requirements-module-definition,Requirements Classes>> and <<ctm-definition,Conformance Classes>> that appear in this document are denoted by partial URIs that are relative to this base.
...
The link relationships used in Common API-4: Discovery are described in <<link-relations-table>>. Additional relation types may be used if the implementation warrants it.
...
|`https://www.opengis.net/def/rel/ogc/1.0/children`           |Indicates that the link target is the list of children resources. [OGC]
...
The Common API-4: Discovery Standard does not mandate the use of a specific coordinate reference system. However, if no CRS is specified, the default coordinate reference systems for spatial geometries are:
...
An <<rfc5234,ABNF>> representation of the RFC 3339 format is provided in https://docs.ogc.org/DRAFTS/20-024.html#date-time-bnf-annex[Annex D of OGC API - Common - Part 2].

Temporal instants and intervals are measured relative to an underlying temporal reference system (TRS).
This Common API-4: Discovery Standard does not mandate a specific temporal coordinate reference system.
...
The OGC Common API - 4: Discovery specifies requirements and recommendations for the development of APIs allowing to list and describe spatial resources using a standard way of doing so.
...
The OGC Common API - 4: Discovery uses OpenAPI 3.0 fragments in examples and to formally state requirements. Using OpenAPI 3.0 is not required for implementing an OGC API Standard.
...
They are available from the OGC Schemas Registry (or GitHub pending publication) at https://github.com/opengeospatial/ogcapi-common/tree/master/discovery/openapi,
https://schemas.opengis.net/ogcapi/common/part1/1.0[https://schemas.opengis.net/ogcapi/common/part1/1.0] and
