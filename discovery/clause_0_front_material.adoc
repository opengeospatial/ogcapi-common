...
This document, OGC API — Common — Part 4: Discovery Within Many Collections Standard (Common API-4: Discovery) is one
of those parts. This part specifies query parameters to discover collections of interest within a Web API deployment serving several collections.
...
This document defines the _OGC API - Common - Part 4: Discovery Within Many Collections_ Standard.
...
A portion of this document has been derived from OGC API - Features - Part 1: Core and OGC API - Records - Part 1: Core
and adapted specifically to discover OGC API collections.
The `query` query parameter of the Searchable Collections requirements class was inspired from the
https://github.com/stac-api-extensions/freetext-search[STAC free text search extension].
Since the content was actually embedded within this document, these are not cited as a normative reference,
but conformance to OGC API - Records can be explicitly declared when applicable.
...
|Francis Charette-Migneault      |Technologies Luqia
|===
