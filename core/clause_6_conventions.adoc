...
The "rel" property is populated using values from the https://www.iana.org/assignments/link-relations/link-relations.xhtml[IANA Link Relations Registry] wherever possible. Additional values are registered with the https://github.com/opengeospatial/NamingAuthority/blob/master/incubation/linkRelationTypes/linkrelations.csv[OGC Link Relation Registry]. Additional relation type values can be used if neither of these registers suffice.

The link relationships used in API-Common Core are described in <<link-relations-table>>. Additional relation types may be used if the implementation warrents it.
