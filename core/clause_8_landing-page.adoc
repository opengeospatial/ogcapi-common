...
[cols="30,20,15,40",options="header"]
!===
^|Resource ^|URI ^|HTTP Method ^|*Description*
|Landing page |`/` ^|GET |the <<landing-page,landing page>>
|API definition |`/api` ^|GET |the <<api-definition,API Definition>> document for this API
|Conformance declaration |`/conformance` ^|GET |the <<conformance-classes,conformance>> information for this API
!===
...
The landing page returned by this operation is based on the following link:http://beta.schemas.opengis.net/ogcapi/common/part1/0.1/core/openapi/schemas/landingPage.json[JSON schema].

.landingPage.json
[source, JSON]
include::openapi/schemas/landingPage.json[]

It is recommended that OGC Web APIs provide a set of Service Metadata. That metadata should identify the service and provide information about the service provider.     

include::recommendations/landing-page/REC_service-metadata.adoc[]
...
The Conformance Declaration resource returned by this operation is based on the following link:http://beta.schemas.opengis.net/ogcapi/common/part1/0.1/core/openapi/schemas/confClasses.json[Conformance Declaration Schema]. 
