...
[source,yaml]
----
include::../core/openapi/schemas/link.yaml[]
----
...
"The Locator Attribute (href) supplies the data that allows an application to find a remote resource or resource fragment. The value of this attribute is an IRI which servers as the Uniform Resource Identifier for the remote resource." (W3C XLink Version 1.1)
...
Relation types are discussed in more detail in <<relation-type-section,Relation Types>>.
...
Extension relation types used in OGC API Standards are registered at https://github.com/opengeospatial/NamingAuthority/blob/master/registers/linkrelations.csv[https://github.com/opengeospatial/NamingAuthority/blob/master/registers/linkrelations.csv] 
