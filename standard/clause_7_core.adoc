...
Figure 1 illustrates the resources supported by the _Core_ requirements class using UML. Each resource type available through the server is an «interface».

A server that implements an OGC API provides access to a collection of resources. In other words, the API is a distribution of that collection. A file download, for example, would be another distribution.
...
Accessing the `Conformance` using HTTP GET returns a list of URIs of requirements classes implemented by the API.
...
A `bbox` or `time` parameter may be used to select only a subset of the resouces in the collection (the resources that are located in the bounding box or time period).

The `limit` parameter may be used to request only a subset of the selected resources and to indicate that the client wants to page through the selected resources of the collection.
...
If the path parameter `collectionId` or the path parameter `featureId` does not exist on
