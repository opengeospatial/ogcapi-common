...
An OGC API provides a lighweight interface to access one or more resources. The resources addressed by OGC APIs fall into three categories; Foundation Resources, Spatial Resources, and Information Resources. These Resource Catagories are described in section <<rc_core-section,8>>, Requirement Class Core.
...
|Landing page |`/` ^|GET |<<landing-page,7.2.1 API Landing Page>>
|API definition |`/api` ^|GET |<<api-definition,7.2.2 API Definition>>
|Conformance classes |`/conformance` ^|GET |<<conformance-classes,7.2.3 Declaration of Conformance Classes>>
|Collections metadata |`/collections` ^|GET |<<collections-metadata,8.2.1 Collections Metadata>>
!===
