...
Foundation Resources are those resource types which are common across all OGC APIs. Those resources are defined in this OGC API-Common standard. Other OGC API standards re-use these resources and, where necessary, extend them to address unique requirements.

Geospatial Resources are the resource types which we usually think of as Geospatial Datasets. They include Features, Coverages, Maps, and Styles. This Standard defines basic patterns for accessing Geospatial Resources, but no detailed requirements. Additional OGC API Standards have been developed to address specific API requirements for each Geospatial Resource type.
...
OGC API hyperlinks are defined using the following link:https://raw.githubusercontent.com/opengeospatial/OAPI_Common/master/openapi/schemas/link.yaml[Hyperlink Schema].
...
Geospatial Resources can be exposed using the path template.
 ...
 {collectionId} = a unique identifier for a resource collection (dataset)
       and
 {itemId} = a unique identifier for a geospatial object within the dataset.
...
|"collections/{collectionId}/items/{itemId}" |A single element from {collectionId} with the identifier {itemId}.
|===
...
OGC APIs typically organize their geospatial resources into collections or datasets. These datasets are accessed through the /collections path.
...
A collection resource is the actual resource as opposed to metadata about that resource. This standard defines the general behavior of this operation, but detailed requirements are the purvue of the API standard for that resource type.
