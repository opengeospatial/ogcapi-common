...
An OGC API provides a lighweight interface to access one or more resources. The resources addressed by OGC APIs fall into three categories; Foundation Resources, Spatial Resources, and Information Resources. These Resource Catagories are described in section <<core-overview,7 Requirement Class "Core">>.
...
|Landing page |`/` ^|GET |<<_api_landing_page,7.2.1 API Landing Page>>
|API definition |`/api` ^|GET |<<_api_definition,7.2.2 API Definition>>
|Conformance classes |`/conformance` ^|GET |<<_declaration_of_conformance_classes,7.2.3 Declaration of Conformance Classes>>
|Collections metadata |`/collections` ^|GET |<<_collections_metadata,8.2.1 Collections Metadata>>
!===
