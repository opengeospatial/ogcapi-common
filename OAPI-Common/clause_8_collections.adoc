...
Hypermedia Access was described in the <<navigation-introduction>> section of Clause 7. For Spatial Resources, hypermedia navigation is enabled through the links included in each schema defined by this Reqirement Class. 
...
The resources returned from each node in this template are described in <<spatial_resource_paths>>.

[#spatial_resource_paths,reftext='{table-caption} {counter:table-num}']
.Spatial Resource Paths
[width="90%",cols="2,4",options="header"]
|===
^|**Path Template** ^|**Resource**
|/collections |Metadata describing the spatial collections avalable from this API.
|/collections/{collectionId} |Metadata describing the collection with the unique identifier `{collectionId}`
|/collections/{collectionId}/item |The spatial collection resource identified by the `{collectionId}` parameter.
|===
...
|===
^|**Path Template** ^|**Resource**
|/collections |The root resource describing the spatial collections avalable from this API.
