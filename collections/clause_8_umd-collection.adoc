...
For additional dimensions beyond spatial and temporal, the reference system is defined as combination of the `definition` and `unit`, or alternatively the `trs` or `vrs`.
...
Unlike the spatial CRS, the collection description in this standard only supports defining a single reference system the temporal dimension and each additional dimensions.
