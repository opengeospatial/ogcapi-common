...
A Uniform Multi-Dimension Collection is an extension of the <<collection-resource-definition-section,Collection>> resource.
...
.JSON OtherDimensionlExtent Properties
...
| **definition**
| String (URI)
| A URI for the definition of the measured or observed property corresponding to this dimension
...
| Temporal Coordinate Reference System (e.g. as defined by Features for 'temporal'). This should be a URI to a registered TRS if one is available, or a full
...
| One or more intervals that describe the extent for this dimension of the dataset. The value `null` is supported and indicates an unbounded or half-bounded
...
| **vrs**
| String (URI, CURIE or WKT2CRS) or object (PROJJSON or CRS JSON)
a| Vertical Coordinate Reference System (e.g. as defined in EDR for 'vertical'). This should be a URI to a registered VRS if one is available, or a full
