[[ats_collections_rc-limit-unsupported]]
[width="90%",cols="2,6a"]
|===
^|*Abstract Test {counter:ats-id}* |*/ats/collections/rc-limit-unsupported*
^|Test Purpose |Validate that each collection provided by the server is described in the Collections Metadata.
^|Requirement |<<req_collections_rc-limit-unsupported,/req/collections/rc-limit-unsupported>>
