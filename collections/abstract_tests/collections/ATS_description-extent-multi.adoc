...
. Verify that for `temporal` properties (as well as for additional properties if conforming to Uniform Additional Dimensions Collections) having more than a single interval, all intervals are within the first interval.
