...
This Requirements Class does not define any parameters for use against a `collections` resource. Implementors who wish to support filtering of the collections to be included in a result set should implement the   <<rc-simple-query-section,Simple Query>> Conformance Class for that purpose.
