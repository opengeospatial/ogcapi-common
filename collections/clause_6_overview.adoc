...
but a list of properties identified as being in use by two or more OGC API Standards, without being tied to any particular data access mechanism.
...
| **links**
| Array of https://docs.ogc.org/is/19-072/19-072.html#link-conventions[Link]
...
| AttributionMediaTypeCode (`text/plain`, `text/html`, `text/markdown`)
| Media type for the markup language of the attribution: It can be either plain text, HTML or CommonMark
...
| **parent**
| String
| An `id` of a collection that is the parent of this collection in a hierarchical relation. For example a rivers’ collection can have a topographic collection as parent
...
| **contacts**
| Array of https://docs.ogc.org/is/20-004r1/20-004r1.html#sc_contacts[Contact]
...
| **resourceLanguages**
| Array of https://docs.ogc.org/is/20-004r1/20-004r1.html#sc_record_language_handling[ResourceLanguage]
...
| **themes**
| Array of https://docs.ogc.org/is/20-004r1/20-004r1.html#sc_keywords_and_themes[Theme]
...
| **featureTypes**
| String
| The list of feature (or coverage) type(s) which are present inside the collection. This is useful for associating the relevant portrayal rules of a style.

| **formats**
| Array of https://docs.ogc.org/is/20-004r1/20-004r1.html#sc_formats[Format]
...
| **crs**
| Array of String (URI, CURIE or https://www.opengis.net/doc/is/crs-wkt/2.1.11[WKT2CRS]) or Object (https://proj.org/en/stable/specifications/projjson.html[PROJJSON] or https://github.com/opengeospatial/CRS-JSON-Encoding[CRS JSON])
...
| **minCellSize**
| Real
| Minimum cell size for usage of the collection, in degrees of latitude ^1^ for data referenced to Earth
| **maxCellSize**
| Real
| Maximum cell size for usage of the collection, in degrees of latitude ^1^ for data referenced to Earth
...
3+| ^1^ To convert to meters, the cell size is simply converted from degrees to radians and multiplied by the equatorial radius in meters.
For example, for the WGS84 ellipsoid, metersCellSize = cellSize * Pi / 180 * 6,378,137 m = cellSize * ~111,319.4908 meters / degree.
The cell size is always in degrees to facilitate discovery and comparison between collections with different native CRS (`storageCrs`).
|===
