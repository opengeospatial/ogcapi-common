...
NOTE: The Collections Requirements Class does not define any query parameters for use against the `/collections` resource.
...
For the spatial extent, the `crs` specified in the `spatial` property of the `extent` defines only the coordinate reference system of the `bbox`, `minCellSize` and `maxCellSize`.
...
or a JSON object encoding of the CRS (using the https://proj.org/en/stable/specifications/projjson.html[PROJJSON] schema or a potential https://github.com/opengeospatial/CRS-JSON-Encoding["CRS JSON"] successor if eventually standardized by the OGC) can be specified.
...
The following tables lists all properties defined as part of the collection's extent.
While additional properties beyond the `spatial` and `temporal` defined here are left open by this "Collections" requirements class,
the <<rc-uad-collections-section,Uniform Additional Dimensions Collections>> requirements class defines a specific uniform schema for properties representing additional
dimensions, while also defining the `grid` property which is also applicable to spatiotemporal extents.

[#extent-properties,reftext='{table-caption} {counter:table-num}']
.JSON Collection's Extent Properties
[cols="1,1,3", options="header"]
|===
| Name (as encoded in JSON) | JSON Type | Definition
| **spatial**
| <<spatialextent-properties,SpatialExtent>>
| The spatial extent of the data in the collection.

| **temporal**
| <<temporalextent-properties,TemporalExtent>>
| The temporal extent of the data in the collection.

| **_(any additional property)_**
| <<otherdimensionextent-properties,OtherDimensionExtent>>
| _(if implementing <<rc-uad-collections-section,Uniform Additional Dimensions Collections>>)_ The extent for any additional dimensions beyond those described in temporal and spatial.
|===

[#spatialextent-properties,reftext='{table-caption} {counter:table-num}']
.JSON SpatialExtent Properties
[cols="1,1,3", options="header"]
|===
| Name (as encoded in JSON) | JSON Type | Definition
| **bbox**
| Array of (at least one) Array of (4 or 6) Real
a| One or more bounding boxes that describe the spatial extent of the dataset.
The first bounding box describes the overall spatial extent of the data. All subsequent bounding boxes describe more precise bounding boxes,
e.g., to identify clusters of data. Clients only interested in the overall spatial extent will only need to access the first item in each array.
For data referenced to Earth, this will be either:

* `[ <West longitude>, <South latitude>, <East longitude>, <North latitude> ]` for a CRS84 bounding box, or
* `[ <West longitude>, <South latitude>, <lowest ellipsoidal height in meters>, <East longitude>, <North latitude>, <highest ellipsoidal height in meters> ]`
for a 3-dimensional CRS84h bounding box.
...
This should be a URI to a registered CRS if one is available, or a full description of the CRS otherwise using e.g., a WKT2CRS string, or a PROJJSON,
or CRS JSON object.

| **grid**
| Array of 2 or 3 (<<regulargrid-properties,RegularGrid>> or <<irregulargrid-properties,IrregularGrid>>)
| _(if implementing <<rc-uad-collections-section,Uniform Additional Dimensions Collections>>)_ Grid description providing information about the limited availability of data within the collection
organized as a grid (regular or irregular) along each spatial dimension.
All units within the spatial grid object are specified in the `storageCrs`, defaulting to CRS84 if not specified.
|===

[#temporalextent-properties,reftext='{table-caption} {counter:table-num}']
.JSON TemporalExtent Properties
[cols="1,1,3", options="header"]
|===
| Name (as encoded in JSON) | JSON Type | Definition
...
The first time interval describes the overall temporal extent of the data. All subsequent time intervals describe more precise time intervals, e.g.,
to identify clusters of data. Clients only interested in the overall extent will only need to access the first item in each array.
The end temporal instant of the interval (second value of each array) includes the thickness of the last time component specified.
For example, the interval `[ "2026-07-25T00:00:00Z", "2026-07-25T23:59:59Z" ]` ends at the very end of the day, including the full thickness of the last second.
Conversely, specifying `[ "2026-07-25T00:00:00Z", "2026-07-26T00:00:00Z" ]` would include the whole first second of the next day.
Note that RFC 3339 explicitly disallows hour 24, but if such a value is encountered for an end time, no thickness should be assumed in that particular case.

| **trs**
| String (URI, CURIE or https://www.opengis.net/doc/is/crs-wkt/2.1.11[WKT2CRS]) or Object (https://proj.org/en/stable/specifications/projjson.html[PROJJSON] or https://github.com/opengeospatial/CRS-JSON-Encoding[CRS JSON])
| Coordinate reference system of the coordinates in the temporal extent (property `interval`).
The default reference system is the Gregorian calendar. For data for which the Gregorian calendar is not suitable, such as geological time scale,
another temporal reference system may be used. This should be a URI to a registered TRS if one is available, or a full description of the TRS otherwise
using e.g., a WKT2CRS string, or a PROJJSON, or CRS JSON object.

| **grid**
| <<regulargrid-properties,RegularGrid>> or <<irregulargrid-properties,IrregularGrid>>
| _(if implementing <<rc-uad-collections-section,Uniform Additional Dimensions Collections>>)_ Grid description providing information about the limited availability of data within the collection organized as a grid (regular or irregular) along the
temporal dimension.
|===
...
This requirement class is consistent with the requirements of https://docs.ogc.org/is/20-004r1/20-004r1.html#clause-local-resources-catalog[OGC API - Records - Part 1: Core "Local Resources Catalog" (Deployment)].
