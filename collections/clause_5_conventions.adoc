...
|`https://www.opengis.net/def/rel/ogc/1.0/data` (or `data`) |Indicates that the link's context is a distribution of a dataset that is an API and refers to the root resource of the dataset in an API. [OGC]
...
Compact URIs (CURIEs) resolve to full URIs based on deterministic rules.
Safe CURIEs (enclosed within `[` square brackets `]`) should generally be used to distinguish them from non-CURIE identifiers.
The general mapping for CURIEs to URIs defined on the OGC definition server is `<authority>[-<URI type>]:<element>` mapping to `<authority URL>/<URI type>/<authority>/0/<element>`,
with the default `URI type` being `crs` when not specified. For example, `[OGC:CRS84]` maps to `https://www.opengis.net/def/crs/OGC/0/CRS84`, and `[ogc-rel:map]` maps to `https://www.opengis.net/def/rel/ogc/0/map.
Unfortunately, there are a few challenges originating from how the URIs themselves are defined which need to be considered for establishing equivalence between a CURIE and a URI.

- The `/0/` is intended to refer to the latest version or for URI types where the version is not relevant (as should be the case for link relations).
The CURIEs should always be interpreted as referring to the latest version, and the version should be ignored when comparing a CURIE against a URI.
The definition server may or may not support resolving both the `/0/` and a specific version e.g., `/1.0/`.
- `http` and `https` URIs should be considered as equivalent.
- While some OGC definition types use lowercase `ogc` (e.g., link relations `/rel/`), others use uppercase `OGC` (e.g., coordinate reference systems `/crs/`).

WARNING: Because multiple OGC API Standards allow the use of safe CURIEs for link relations and CRS references, OGC API - Common also allows them for compatibility reasons.
However, server implementations conforming to OGC API Standards where CURIEs are not supported need to avoid them and only use full URIs for all resources relevant to those standards.
Client implementations for OGC API Standards allowing the use of CURIEs need to be prepared to accept both CURIEs and full URIs.

While CURIEs themselves avoid the confusion of http vs. https and version numbers, the challenges associated with mapping them to multiple URIs requires more complicated logic
than comparing an identifier string to the CURIE itself and a single full URI.
...
NOTE: The order of coordinates for the bounding boxes and spatial intervals used within the collection description will always follow the axis order of the applicable CRS exactly.

OGC Abstract Specification Topic 2 – Referencing by Coordinates  (<<iso19111,ISO 19111>>) provides the conceptual and logical model for Coordinate Reference Systems.
...
A server implementation of the OGC API – Common - Part 2 Standard can use the content in the `openapi` directory to generate a response for the API description
using OpenAPI 3.0. The `ogcapi-common-2.yaml` file includes paths and components. An implementation should adapt their definition to include any additional
resource paths or query parameters defined by other supported OGC API Standards.
The components part includes parameters, responses and schemas that can be reused as-is.
However, the implementation should select the appropriate collection description schema
(https://github.com/opengeospatial/ogcapi-common/blob/master/collections/openapi/schemas/common-geodata/collectionDesc.yaml[collectionDesc.yaml] or
...
based on whether they implement the <<rc-uad-collections-section,Uniform Additional Dimensions Collections Requirements Class>> or not.
For proper validation against the Uniform Additional Dimensions schemas, both the
https://github.com/opengeospatial/ogcapi-common/blob/master/collections/openapi/schemas/common-geodata/collections.yaml[`collections.yaml`] schema
(describing the list of collections) and the
https://github.com/opengeospatial/ogcapi-common/blob/master/collections/openapi/responses/common-geodata/rCollection.yaml[`rCollection.yaml`]
OpenAPI response for the `/collections/{collectionId}` resource need to be modified to include the UAD version of the collection description.

The `api` directory contains JSON files that are templates with enumerated values for collections.
A particular implementation of the Common API-2: GeoData should enumerate the actual resources exposed by the API deployment in the same way.
The implementation can select to dynamically implement responses to `/api/all-collections` as a Web resource supported by the server, or instead to hardcode
the `api/all-collections.json` file with the actual list of resource identifiers in the enumerations.

To improve performance, the whole content of this directory can be bundled into a single document by executing a tool such as `swagger-cli`.
This document can be served for the OGC API - Common - Part 1 `service-desc` link from the landing page.
