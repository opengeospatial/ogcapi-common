...
part:: When an `extent` property is provided for a collection, that extent's boundaries SHALL encompass the spatial and temporal extent of the data in the collection.
The temporal extent may use `null` values to indicate a half-bounded or unbounded time interval. +
If the data contains spatial or temporal information defined in multiple properties, the Implementation needs to determine how this extent is derived from those properties.
